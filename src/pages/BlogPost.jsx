...
// 1. Import your central data!
import { blogPosts } from "@/lib/blogData"; 

export default function BlogPost() {
  // 2. Grab the ID from the URL (e.g., /blog/1)
  const { id } = useParams();
  
  // 3. Find the post that matches that ID (converting the string ID to a Number)
  const post = blogPosts.find((p) => p.id === parseInt(id));

  // 4. Handle the 404 state if a user types a weird URL like /blog/999
  if (!post) {
    return (
      <div className="min-h-screen bg-background text-foreground flex flex-col">
        ...
        <main className="flex-1 flex flex-col items-center justify-center text-center px-4">
          ...
          <Button asChild><Link to="/blog">Back to all posts</Link></Button>
        </main>
        <Footer />
      </div>
    );
  ...
  // 5. Render the dynamically matched post!
  return (
    <div className="min-h-screen bg-background text-foreground flex flex-col">
      <Navbar />
      ...
      <main className="flex-1 py-16 md:py-24">
        <article className="container mx-auto px-4 md:px-8 max-w-4xl">
          ...
          <header className="mb-10 text-center md:text-left">
            <Badge className="mb-6" variant="secondary">{post.category}</Badge>
          ...
          {/* Map through the content array to generate paragraphs */}
          <div className="prose prose-zinc dark:prose-invert max-w-3xl mx-auto md:text-lg leading-loose text-muted-foreground space-y-8">
            {post.content.map((paragraph, index) => (
              <p key={index} className={index === 0 ? "text-foreground font-medium text-xl leading-relaxed" : ""}>
                {paragraph}
              </p>
            ))}
          </div>
